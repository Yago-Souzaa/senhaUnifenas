...
export async function GET(request: NextRequest, { params }: { params: Params }) {
  ...
  try {
    ...
    const passwordDoc = await passwordsCollection.findOne({ _id: new ObjectId(id), isDeleted: { $ne: true } }) as PasswordEntry | null;

    if (!passwordDoc) {
      return NextResponse.json({ message: 'Password not found or has been deleted' }, { status: 404 });
    }

    // Check ownership or direct share (legacy, will be phased out)
...
export async function PUT(request: NextRequest, { params }: { params: Params }) {
  ...
  try {
    ...
    // Legacy direct share check (being phased out)
